...
class OctConv2D(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.high_to_low_kernel  = self.add_weight(name="high_to_low_kernel", 
                                    ...
                                    constraint=self.kernel_constraint)
        # High -> High_Mlp
        self.to_high_mlpkernel = self.add_weight(name="to_high_mlpkernel",
                                                 shape=(*self.mlpkernel, self.high_channels, self.high_channels),
                                                 initializer=self.kernel_initializer,
        ...
        # Low -> Low_Mlp
        self.to_low_mlpkernel = self.add_weight(name="to_low_mlpkernel",
                                                shape=(*self.mlpkernel, self.low_channels, self.low_channels),
                                                initializer=self.kernel_initializer,
                                                regularizer=self.kernel_regularizer, constraint=self.kernel_constraint)
    ...
    def call(self, inputs):
        ...
        # High_add -> High_add mlp
        high_add   = K.conv2d(high_add, self.to_high_mlpkernel,
                                strides=self.strides, padding=self.padding,
                                data_format="channels_last")
        low_add = high_to_low + low_to_low
        # Low_add -> Low_add mlp
        low_add   = K.conv2d(low_add, self.to_low_mlpkernel,
                                strides=self.strides, padding=self.padding,
                                data_format="channels_last")
