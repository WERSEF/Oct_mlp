...
def _create_octconvmlp_residual_block(inputs, ch, N, alpha):
    ...
    for i in range(N):
        ...
        high, low = oct_convmlp.OctConv2D(filters=ch, alpha=alpha)([high, low])
        high = layers.BatchNormalization()(high)
        high = layers.Activation("relu")(high)
        low = layers.BatchNormalization()(low)
        low = layers.Activation("relu")(low)
...
def create_octconvmlp_wide_resnet(alpha, N=4, k=10):
    ...
    # 16 channels block
    high, low = oct_convmlp.OctConv2D(filters=16, alpha=alpha)([input, low])
    high = layers.BatchNormalization()(high)
    high = layers.Activation("relu")(high)
    low = layers.BatchNormalization()(low)
    low = layers.Activation("relu")(low)
...
def create_octcovmlp_vgg(alpha):
    ...
    # Block 1
    high, low = oct_convmlp.OctConv2D(filters=64, alpha=alpha, kernel_size=(3, 3), activation='relu', padding='same')([input, low])
    high, low  = oct_convmlp.OctConv2D(filters=64, alpha=alpha, kernel_size=(3, 3), activation='relu', padding='same')([high, low ])

    # Block 2
    high, low = oct_convmlp.OctConv2D(filters=128, alpha=alpha, kernel_size=(3, 3), activation='relu', padding='same')([high, low])
    high, low = oct_convmlp.OctConv2D(filters=128, alpha=alpha, kernel_size=(3, 3), activation='relu', padding='same')([high, low])
    high = layers.MaxPooling2D((2, 2), strides=(2, 2))(high)
    low = layers.MaxPooling2D((2, 2), strides=(2, 2))(low)

    # Block 3
    high, low = oct_convmlp.OctConv2D(filters=256, alpha=alpha, kernel_size=(3, 3), activation='relu', padding='same')([high, low])
    high, low = oct_convmlp.OctConv2D(filters=256, alpha=alpha, kernel_size=(3, 3), activation='relu', padding='same')([high, low])
    high, low = oct_convmlp.OctConv2D(filters=256, alpha=alpha, kernel_size=(3, 3), activation='relu', padding='same')([high, low])
    high = layers.MaxPooling2D((2, 2), strides=(2, 2))(high)
    low = layers.MaxPooling2D((2, 2), strides=(2, 2))(low)
    ...
    # Block 4
    high, low = oct_convmlp.OctConv2D(filters=512, alpha=alpha, kernel_size=(3, 3), activation='relu', padding='same')([high, low])
    high, low = oct_convmlp.OctConv2D(filters=512, alpha=alpha, kernel_size=(3, 3), activation='relu', padding='same')([high, low])
    high, low = oct_convmlp.OctConv2D(filters=512, alpha=alpha, kernel_size=(3, 3), activation='relu', padding='same')([high, low])
    high = layers.MaxPooling2D((2, 2), strides=(2, 2))(high)
    low = layers.MaxPooling2D((2, 2), strides=(2, 2))(low)
